...
function BriefcaseIcon() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2M12 12v4M10 14h4" /></svg>
}
function ZapIcon() {
...
const metricColors = [
  ...
  { bg: 'bg-success-light', text: 'text-[#16A34A]', border: 'border-[#BBF7D0]' },
    ...
    >
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-2xl font-semibold text-primary tracking-tight">{profile.name}</h1>
                <span className="tag-success text-[11px]">
                  <span className="status-dot w-1.5 h-1.5" />
                  {profile.status}
                </span>
              </div>
              <p className="text-sm text-accent font-medium mt-0.5">{profile.role}</p>
              <p className="text-sm text-muted mt-2 max-w-md leading-relaxed">{profile.tagline}</p>
            </div>
          </div>
        ...
        {metrics.map((metric, i) => {
          ...
          return (
            <motion.div key={metric.label} variants={item}>
              <div className={`metric-card border ${colors.border} transition-all duration-200 hover:shadow-panel-hover`}>
                <div className={`w-7 h-7 rounded-lg ${colors.bg} flex items-center justify-center ${colors.text}`}>
                  {Icon && <Icon />}
                </div>
                <div className="mt-2">
                  <p className={`text-2xl font-semibold tracking-tight ${colors.text}`}>{metric.value}</p>
      ...
      <motion.div variants={item} className="panel p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { label: 'Availability', value: 'Open to new clients', status: 'active' },
            { label: 'Work Mode', value: 'Remote — Async friendly', status: 'active' },
            { label: 'Commitment', value: 'Part-time & Full-time', status: 'neutral' },
          ].map((s) => (
            <div key={s.label} className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[#F8FAFC] border border-border">
              <span className={`w-2 h-2 rounded-full shrink-0 ${s.status === 'active' ? 'bg-success' : 'bg-amber-400'}`} />
              <div>
